...
# --- Functions from dab-yeet-su-scraper-clean (for self-containment) ---
...
def get_json_response(url: str) -> dict:
    ...
    """
    response = rq.get(url, headers=HEADERS, timeout=10) # Added timeout for robustness
    response.raise_for_status() # Raise HTTPError for bad responses (4xx or 5xx)
    return response.json()

# Re-introducing get_track_info for internal use within getTopSongs
...
def getTopSongs():
    ...
    for song_soup in song_containers:
        ...
        try:
            ...
            # --- NEW LOGIC: Get track_id and audioQuality from dab.yeet.su for this song ---
            if title and artist:
        ...
        if title and artist:
            top_songs_data.append({
                'title': title,
                'artist': artist,
                'id': track_id, # Include the track_id
                'audioQuality': audio_quality, # Include audioQuality
